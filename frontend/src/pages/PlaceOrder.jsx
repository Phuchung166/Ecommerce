    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      ...
      <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
        ...
        <div className='flex gap-3'>
          ...
          <input required onChange={onChangeHandler} name='ward' value={formData.ward} className='border border-gray-300 rounded py-1.5 px-3.5 w-full' type="text" placeholder='Phường/Xã' />
        </div>
        ...
        <div className='mt-12'>
          ...
          <div className='flex gap-3 flex-col lg:flex-row'>
            <div onClick={() => setMethod('stripe')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              ...
              <img className='h-6 mx-4 scale-150 object-contain' src={assets.vietqr_logo} alt="" />
            </div>
            <div onClick={() => setMethod('razorpay')} className='flex items-center gap-3 border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-400' : ''}`}></p>
              <img className='h-6 mx-4 scale-150 object-contain' src={assets.momo_logo} alt="" />
            </div>
          ...
          <div className='w-full text-end mt-8'>
            <button type='submit' className='bg-[#EE4D2D] text-white px-10 py-3 text-sm cursor-pointer'>THANH TOÁN</button>
          </div>
